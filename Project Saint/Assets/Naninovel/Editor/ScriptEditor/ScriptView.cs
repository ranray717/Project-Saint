...
namespace Naninovel
{
    public class ScriptView : VisualElement
    {
        ...
        public static StyleSheet DarkStyleSheet { get; private set; }
        ...
        private void HandleKeyDownEvent (KeyDownEvent evt)
        {
            if (evt != null)
            {
                if (evt.keyCode == config.InsertLineKey && (evt.modifiers & config.InsertLineModifier) != 0) { ShowSearcher(); evt.StopImmediatePropagation(); }
            ...
            {
                if (Event.current.keyCode == config.InsertLineKey && Event.current.modifiers == config.InsertLineModifier) { ShowSearcher(); Event.current.Use(); }
        ...
        private void HandleMouseDownEvent (MouseDownEvent evt)
        {
            ...
            var player = Engine.GetService<IScriptPlayer>();
            var stateMngr = Engine.GetService<IStateManager>();
            if (stateMngr is null || player is null || !player.PlayedScript || player.PlayedScript.Name != scriptAsset.name) return;
            ...
            if (rewindIndex < player.PlaybackSpot.LineIndex)
            {
                ...
                bool CanRollback () => stateMngr.CanRollbackTo(s => s.PlaybackSpot.ScriptName == player.PlaybackSpot.ScriptName && s.PlaybackSpot.LineIndex == rewindIndex);
